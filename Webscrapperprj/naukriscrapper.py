...
def get_jobs(page_url):
    ...
    for job in all_jobs:
        ...
        try:
            job_title = job.find('div', {'class': 'job-tittle'}).find('h3', {'class': 'medium'}).find('a', {'href': True}).text
            job_company = job.find('div', {'class': 'job-tittle'}).find('span', {'class': 'company-name'}).find('a', {'href': True}).text
            # get all the sub section 'span' classes to get the place, experience and package details
            job_pep = job.find('div', {'class': 'job-tittle'}).find('div', {'class': 'searctag row'}).find_all('span', {'class': 'loc'})
